...
class calcualte_suction_score:
    ...
    def calculator(
        ...
        if self.grasps_and_predictions != None:
            ...
            self.suction_coordinates = torch.mm(
                ...
                euler_angles_to_matrix(
                    centroid_angle_suction.clone().detach().to(self.device),
                    "XYZ",
                    degrees=False,
                ).type(torch.float64),
                out=None,
            ...
            if self.segmask[V[i].type(torch.int)][U[i].type(torch.int)] == 0:
                if grasps_and_predictions == None:
                    return (
                        ...
                        torch.tensor([0, 0, 0]),
                        torch.tensor(
                            [centroid_angle[0], centroid_angle[1], centroid_angle[2]]
                        ),
                    )
                else:
                    return (
        ...
        point_cloud_suction_facing_normal = torch.mm(
            ...
            euler_angles_to_matrix(
                -torch.tensor([-centroid_angle[1], centroid_angle[0], 0.0]).to(
                    self.device
                ),
                "XYZ",
                degrees=False,
    ...
    def calculate_contact(
        ...
        if (
            furthest_suction_projection_point > 0.04
