...
class calcualte_suction_score():
    ...
    def convert_rgb_depth_to_point_cloud(self):
        camera_u = torch.arange(
            0, self.camera_intrinsics.width, device=self.device)
        camera_v = torch.arange(
            0, self.camera_intrinsics.height, device=self.device)
        ...
        position = torch.vstack(
            (X, Y, Z, torch.ones(len(X), device=self.device)))[:, valid]
    ...
    def find_nearest(self, centroid, points):
        ...
        distances = torch.cdist(points[:, :2].type(
            torch.float64), suction_points.type(torch.float64))
    ...
    def calculator(self, depth_image, segmask, rgb_img, grasps_and_predictions, object_id, offset):
        ...
        point_cloud_im = self.camera_intrinsics.deproject_to_image(
            depth_normal)
        ...
        centroid_point = torch.FloatTensor([torch.median(points[:, 0]), torch.median(
            points[:, 1]), torch.median(points[:, 2])]).to(self.device)
        ...
        if (self.grasps_and_predictions == None):
            ...
            xyz_point = self.convert_uv_point_to_xyz_point(
                self.grasps_and_predictions.center.x+offset[0], self.grasps_and_predictions.center.y+offset[1])
        if (xyz_point[2] < 0):
            return torch.tensor(0), torch.tensor([0, 0, 0]), torch.tensor([centroid_angle[0], centroid_angle[1], centroid_angle[2]])
        ...
        for angle in range(45, 360, 45):
            ...
            '''
            self.suction_coordinates = torch.cat((self.suction_coordinates, torch.tensor(
                [[x, y, 0.]]).to(self.device)), dim=0).type(torch.float64)
        if (self.grasps_and_predictions != None):
            centroid_angle = torch.tensor(self.normal_cloud_im.data[int(
                self.grasps_and_predictions.center.y + offset[1])][int(self.grasps_and_predictions.center.x + offset[0])]).to(self.device)
            centroid_angle[2] = 0
            # Did trial and error to obtain this transformation
            rotation_matrix_normal = euler_angles_to_matrix(
                torch.tensor([0, 0, -90]).to(self.device), "XYZ", degrees=True)
            null_translation = torch.tensor([0, 0, 0]).to(self.device)
            T_normal = transformation_matrix(
                rotation_matrix_normal, null_translation)
            rotation_matrix_suction = euler_angles_to_matrix(
                centroid_angle.clone().detach().to(self.device), "XYZ", degrees=False)

            null_translation = torch.tensor([0, 0, 0]).to(self.device)
            T_suction = transformation_matrix(
                rotation_matrix_suction, null_translation)
            '''
            ...
            centroid_angle_suction = matrix_to_euler_angles(
                T_suction_to_normal[:3, :3], "XYZ")
            centroid_angle_suction[2] = 0.0
            self.suction_coordinates = torch.mm(self.suction_coordinates, euler_angles_to_matrix(
                centroid_angle_suction.clone().detach().to(self.device), "XYZ", degrees=False).type(torch.float64), out=None)
        else:
            centroid_angle = torch.tensor(self.normal_cloud_im[int(
                self.camera_intrinsics.height/2)][int(self.camera_intrinsics.width/2)]).to(self.device)
            centroid_angle[2] = 0
        '''
        ...
        minimum_suction_point = torch.min(
            point_cloud_suction_facing_normal[:, 2]).to(self.device)
        ri = torch.clamp(torch.abs(
            point_cloud_suction_facing_normal[:, 2] - minimum_suction_point) / 0.023, max=1.0)
        suction_score_facing_normal = 1-torch.max(ri)

        minimum_suction_point = torch.min(
            point_cloud_suction[:, 2]).to(self.device)
        ri = torch.clamp(
            torch.abs(point_cloud_suction[:, 2] - minimum_suction_point) / 0.023, max=1.0)
    ...
    def calculate_contact(self, depth_image, segmask, rgb_img, grasps_and_predictions, object_id):
        ...
        for i in range(8):
            if (torch.isnan(U[i]) or torch.isnan(V[i]) or self.depth_image[V[i].type(torch.int)][U[i].type(torch.int)] == 0):
                continue
            if (furthest_suction_projection_point < self.depth_image[V[i].type(torch.int)][U[i].type(torch.int)]):
                furthest_suction_projection_point = self.depth_image[V[i].type(
                    torch.int)][U[i].type(torch.int)]
            if (nearest_suction_projection_point > self.depth_image[V[i].type(torch.int)][U[i].type(torch.int)]):
                nearest_suction_projection_point = self.depth_image[V[i].type(
                    torch.int)][U[i].type(torch.int)]

        if (furthest_suction_projection_point > 0.04 or nearest_suction_projection_point > 0.02):
            return torch.tensor(0)
        ...
        for i in range(8):
            if (torch.isnan(U[i]) or torch.isnan(V[i])):
                count += 1
                continue
            if ((furthest_suction_projection_point - self.depth_image[V[i].type(torch.int)][U[i].type(torch.int)]) > 0.0175 or self.depth_image[V[i].type(torch.int)][U[i].type(torch.int)] > 0.02):
                count += 1
