...
for angle in range(45, 360, 45):
    ...
    """
    suction_coordinates = np.concatenate(
...
plt.grid()
plt.plot(
    suction_coordinates[:, 1],
    suction_coordinates[:, 0],
    marker="o",
    markersize=2,
    markeredgecolor="red",
...
angle_y = y_a
trasnformed_points = (
    xyz_points[:, 2] * math.cos(np.deg2rad(angle_x)) * math.cos(np.deg2rad(angle_y))
    - suction_coordinates[:, 2]
)
