...
pick_len = 3
update_size = pick_len * 20000 #110 #20
...
env_name = "RL_UR16eManipulation_Full_Nocam"#"RL_UR16eManipulation_Full"
policy_name = "retract_objL_fixed_sucFLgBug"#"random_scene_retract_sim_act"
head_less = False
EVAL = False #if you want to evaluate the model
action_std = 1e-9        # starting std for action distribution (Multivariate Normal)
load_policy = True
policy_name = "Nocam_state_fixedscene_contreward_batch_270_lra_1e-07_lrc_1e-07_clip0.13"
# policy_name = "{}_batch_{}_lra_{}_lrc_{}_clip{}".format(policy_name, update_size, lr_actor, lr_critic, eps_clip)
load_policy_version = 21                   # specify policy version (i.e. int, 50) when loading a trained policy
...
if not EVAL:
    ...
    wandb.define_metric("Total dones", step_metric="custom_step")
...
policy_num = 0
if load_policy:
    policy_num = load_policy_version + 1
...
while time_step <= max_training_timesteps: ## prim_step
    ...
    # 1st item is discrete action (0-5), 2nd item is continuous action [0, 1]
    # action = torch.tensor(action.shape[0] * [[torch.randint(0, 6, (1,)), torch.rand(1)]]).to(sim_device)
    # action = torch.tensor(action.shape[0] * [[5, torch.rand(1)]]).to(sim_device)
    ...
    for i, true_i in enumerate(true_indicies):
        if len(buf_envs[true_i].rewards) != len(buf_envs[true_i].states):
            ...
            # print("reward[i].item()", reward[i].item())
            if reward[i].item() > 0.8:
                total_grasp_success += 1
            if reward[i].item() > 1.8:
        ...
        if buf_envs[true_i].is_done():
            ...
            if len(buf_envs[true_i].rewards) == len(buf_envs[true_i].states):
                ...
                total_dones += 1
                if not EVAL:
                    # wandb.log({"Total dones": total_dones})
                    # wandb.log({"Central buffer size": len(buf_central.states)})
                    # wandb.log({"Total grasp success": total_grasp_success})
                    # wandb.log({"Total extract success": total_extract_success})
                    log_dict = {
                        "Total dones": total_dones,
                        "Central buffer size": len(buf_central.states),
                        "Total grasp success": total_grasp_success,
                        "Total extract success": total_extract_success
                    }
                    wandb.log(log_dict)
