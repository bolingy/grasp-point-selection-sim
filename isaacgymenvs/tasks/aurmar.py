...
class Aurmar(VecTask):
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.num_props = 1#self.cfg["env"]["numProps"]
        ...
        with open('/home/bolingy/Aurmr_Project_WS/aurmr_storm/content/configs/gym/collision_primitives_3d.yml') as file:
        ...
        self.franka_default_dof_pos = to_torch([0.06, -2.5, 2.03, 0.58, 1.67, 1.74], device=self.device) #0.469, 0.035, 0.035],
        ...
        self.itr = 0 ############### temp variable for dev
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_options2.flip_visual_attachments = False
        asset_options2.fix_base_link = True
        asset_options2.collapse_fixed_joints = True
        asset_options2.disable_gravity = False
        #asset_options2.default_dof_drive_mode = gymapi.DOF_MODE_NONE
        #asset_options2.armature = 0.005
        cabinet_asset = self.gym.load_asset(self.sim, asset_root, cabinet_asset_file, asset_options2)

        franka_dof_stiffness = to_torch([800, 800, 800, 800, 800, 800], dtype=torch.float,
                                        device=self.device) #, 400, 1.0e6, 1.0e6
        franka_dof_damping = to_torch([40, 40, 40, 40, 40, 40], dtype=torch.float, device=self.device) #, 80, 1.0e2, 1.0e2
        ...
        franka_start_pose.p =  gymapi.Vec3(2.208, 0.002, 2.020)# 0.208, 2.020, 0.002
        from scipy.spatial.transform import Rotation as R
        r = R.from_euler('zyx', [180, 0, 180], degrees=True).as_quat()
        franka_start_pose.r = gymapi.Quat(r[0], r[1], r[2], r[3])
        ...
        for i in range(self.num_envs):
            ...
            hand_handle = self.gym.find_actor_rigid_body_handle(env_ptr, franka_actor, self.franka_hand)
            ...
            hand_idx = self.gym.find_actor_rigid_body_index(env_ptr, franka_actor, self.franka_hand, gymapi.DOMAIN_SIM)
            ...
            table_pose.p = gymapi.Vec3(0., 0., 1.)
            table_pose.r = gymapi.Quat(1, 0, 0, 0)
            #table_asset = self.gym.create_box(self.sim, 0.5, 0.5, 0.4, asset_options)
                          #self.gym.create_box(self.sim, self.prop_width, self.prop_height, self.prop_width, box_opts)
            #table_handle = self.gym.create_actor(env_ptr, table_asset, table_pose, 'table', i, 0, 0)s
            ...
            if self.num_props > 0:
                ...
                for j in range(props_per_row):
                    ...
                    for k in range(props_per_row):
                        ...
                        prop_state_pose.p.x = propx# drawer_pose.p.x + propx
                        propz, propy = 0, prop_up
                        prop_state_pose.p.y = propy # drawer_pose.p.y + propy
                        prop_state_pose.p.z = propz# drawer_pose.p.z + propz
        ...
        self.hand_handle = self.gym.find_actor_rigid_body_handle(env_ptr, franka_actor, self.franka_hand)
        ...
        self.init_data()
        self.init_ef_ctrl(franka_asset)

    def init_ef_ctrl(self, franka_asset):
        # initial hand position and orientation tensors
        self.init_pos = torch.Tensor(self.init_pos_list).view(self.num_envs, 3).to(self.device)
        self.init_orn = torch.Tensor(self.init_orn_list).view(self.num_envs, 4).to(self.device)
        # desired hand positions and orientations
        self.pos_des = self.init_pos.clone()
        self.orn_des = self.init_orn.clone()
        # Prepare jacobian tensor
        # For franka, tensor shape is (num_envs, 10, 6, 9)
        _jacobian = self.gym.acquire_jacobian_tensor(self.sim, "franka")
        jacobian = gymtorch.wrap_tensor(_jacobian)
        # Jacobian entries for end effector
        hand_index = self.gym.get_asset_rigid_body_dict(franka_asset)[self.franka_hand]
        self.j_eef = jacobian[:, hand_index - 1, :]
        ...
        self.dof_vel = dof_states[:, 1].view(self.num_envs, -1, 1)[:, :self.num_franka_dofs, :]
        self.dof_pos = dof_states[:, 0].view(self.num_envs, -1, 1)[:, :self.num_franka_dofs, :]
    ...
    def add_table(self, table_dims, table_pose, robot_pose, env_ptr, color=[1.0,0.0,0.0]):
        ...
        table_handle = self.gym.create_actor(env_ptr, table_asset, table_pose, 'table', 2, 0, 1)
    ...
    def compute_observations(self):
        ...
        #, self.drawer_grasp_rot[:], self.drawer_grasp_pos[:] drawer_rot, drawer_pos, self.drawer_local_grasp_rot, self.drawer_local_grasp_pos
    ...
    def pre_physics_step(self, actions):
        ...
        pos_cur = self.rb_states[self.hand_idxs, :3]
        orn_cur = self.rb_states[self.hand_idxs, 3:7]
        # Set desired hand positions
        self.pos_des[:, 0] = self.init_pos[:, 0] - 0.1
        self.pos_des[:, 1] = math.sin(self.itr / 50) * 0.2
        self.pos_des[:, 2] = self.init_pos[:, 2] + math.cos(self.itr / 50) * 0.2
        ...
        orn_err = self.orientation_error(self.orn_des, orn_cur)
        pos_err = self.kp * (self.pos_des - pos_cur)
        dpose = torch.cat([pos_err, orn_err], -1)
        u = torch.transpose(self.j_eef, 1, 2) @ m_eef @ (self.kp * dpose).unsqueeze(-1) - \
            self.kv * self.mm @ self.dof_vel
        # Set tensor action
        #zs = torch.zeros((self.num_envs, 4, 1)).to(self.device)
        #u = torch.cat((u, zs), 1)
